...
/*
 * Plugin Name: Easy wp-login.php
 * Plugin URI:  https://www.easywptips.com/easy-wp-login.php
 * Description: We Design Powerful WordPress Admin Dashboard Website URI Change, easy Plugin. Download Today Share.
 * Version:     1.0
 * Author:      @easywptips
 * Author URI:  https://easywptips.com
 * Text Domain: easy-wp-login
 * Domain Path:
 * Network:
 * License:     GPL-2.0+
 */

if ( defined( 'ABSPATH' ) && ! class_exists( 'easy_WP_Login' ) ) {
	...
	class easy_WP_Login {
		...
		public function __construct() {
			...
			register_uninstall_hook( $this->basename(), array( 'easy_WP_Login', 'uninstall' ) );
		...
		public function admin_notices_incompatible() {
			echo '<div class="error"><p>' . sprintf( __( 'Please upgrade to the latest version of WordPress to activate %s.', 'easy-wp-login' ), '<strong>' . __( 'easy wp-login.php', 'easy-wp-login' ) . '</strong>' ) . '</p></div>';
		}
		...
		public function wpmu_options() {
			...
			$out .= '<h3>' . __( 'easy wp-login.php', 'easy-wp-login' ) . '</h3>';
			$out .= '<p>' . __( 'This option allows you to set a networkwide default, which can be overridden by individual sites. Simply go to to the site’s permalink settings to change the url.', 'easy-wp-login' ) . '</p>';
			$out .= '<table class="form-table">';
				$out .= '<tr valign="top">';
					$out .= '<th scope="row">' . __( 'Networkwide default', 'easy-wp-login' ) . '</th>';
		...
		public function admin_init() {
			...
			add_settings_section(
				'easy-wp-login-section',
				__( 'easy wp-login.php', 'easy-wp-login' ),
			...
			add_settings_field(
				'rwl-page',
				'<label for="rwl-page">' . __( 'Login url', 'easy-wp-login' ) . '</label>',
		...
		public function rwl_section_desc() {
			...
			if ( is_multisite() && is_super_admin() && is_plugin_active_for_network( $this->basename() ) ) {
				$out .= '<p>' . sprintf( __( 'To set a networkwide default, go to %s.', 'easy-wp-login' ), '<a href="' . network_admin_url( 'settings.php#rwl-page-input' ) . '">' . __( 'Network Settings', 'easy-wp-login' ) . '</a>') . '</p>';
			}
		...
		public function admin_notices() {
			...
			if ( ! is_network_admin() && $pagenow === 'options-permalink.php' && isset( $_GET['settings-updated'] ) ) {
				echo '<div class="updated"><p>' . sprintf( __( 'Your login page is now here: %s. Bookmark this page!', 'easy-wp-login' ), '<strong><a href="' . $this->new_login_url() . '">' . $this->new_login_url() . '</a></strong>' ) . '</p></div>';
			}
		}

		public function plugin_action_links( $links ) {
			if ( is_network_admin() && is_plugin_active_for_network( $this->basename() ) ) {
				array_unshift( $links, '<a href="' . network_admin_url( 'settings.php#rwl-page-input' ) . '">' . __( 'Settings', 'easy-wp-login' ) . '</a>' );
			} elseif ( ! is_network_admin() ) {
				array_unshift( $links, '<a href="' . admin_url( 'options-permalink.php#rwl-page-input' ) . '">' . __( 'Settings', 'easy-wp-login' ) . '</a>' );
			}
		...
		public function plugins_loaded() {
			...
			load_plugin_textdomain( 'easy-wp-login' );
			...
			if (
				...
				wp_die( __( 'This feature is not enabled.', 'easy-wp-login' ) );
			}
		...
		public function wp_loaded() {
			...
			if ( is_admin() && ! is_user_logged_in() && ! defined( 'DOING_AJAX' ) ) {
				wp_die( __( 'You must log in to access the admin area.', 'easy-wp-login' ) );
			}
